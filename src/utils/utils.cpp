  ...
  while (p != NULL) {
    ...
    if (addr && addr->sa_family == AF_INET) {
      ...
      if (ip != CONSTS_LOCALHOST) {
        coco_trace("retrieve local ipv4 ip=%s, index=%d", ip.c_str(),
                   (int)ips.size());
        ips.push_back(ip);
      }
      ...
      if (!net_device_is_internet(inaddr->s_addr)) {
        coco_trace("detect intranet address: %s, ifname=%s", ip.c_str(),
                   cur->ifa_name);
        _device_ifs[cur->ifa_name] = false;
      } else {
        coco_trace("detect internet address: %s, ifname=%s", ip.c_str(),
                   cur->ifa_name);
...
int FastBuffer::update(char *data, int required_size) {
  ...
  if (nb_free_space < required_size) {
    ...
    if (nb_free_space < required_size) {
      ...
      coco_error("buffer overflow, required=%d, max=%d, left=%d, ret=%d",
...
void FastBuffer::set_buffer(int buffer_size) {
  ...
  if (buffer_size > MAX_SOCKET_BUFFER) {
    coco_warn("limit the user-space buffer from %d to %d", buffer_size,
              MAX_SOCKET_BUFFER);
  }
...
int FastBuffer::realloc_buffer(int _size) {
  ...
  realloc_size =
      coco_max(realloc_size, nb_buffer) + 2 * DEFAULT_RECV_BUFFER_SIZE;
  if (realloc_size > MAX_SOCKET_BUFFER) {
    ret = ERROR_READER_BUFFER_OVERFLOW;
    coco_error("buffer overflow, required=%d, buffer size=%d, left=%d, realloc "
               "size=%d, max realloc size=%d",
               _size, nb_buffer, (int)(buffer + nb_buffer - end), realloc_size,
               MAX_SOCKET_BUFFER);
    return ret;
  }
  ...
  int nb_free_space = (int)(buffer + nb_buffer - end);
  coco_warn("buffer realloc, required=%d, before: max=%d, left=%d, now: "
            "max=%d, left=%d",
            _size, before_buffer_size, before_free_space, nb_buffer,
            nb_free_space);
  return COCO_SUCCESS;
}
