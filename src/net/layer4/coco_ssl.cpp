...
SslServer::SslServer(st_netfd_t _stfd, StreamConn* under_layer) : StreamConn(_stfd)
{
    //  接管under_layer_ skt
...
int SslServer::ReadFully(void* buf, size_t size, ssize_t* nread)
{
    return skt_->ReadFully(buf, size, nread);
}

int SslServer::Read(void* plaintext, size_t nn_plaintext, ssize_t* nread)
{
    int err = COCO_SUCCESS;
...
int SslServer::Write(void* plaintext, size_t nn_plaintext, ssize_t* nwrite)
{
    int err = COCO_SUCCESS;
...
int SslServer::Writev(const iovec *iov, int iov_size, ssize_t* nwrite)
{
    int err = COCO_SUCCESS;
...
std::string SslServer::RemoteAddr() {
