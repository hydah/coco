...
UdpConn::UdpConn(st_netfd_t stfd, struct sockaddr &addr, socklen_t len)
    : DatagramConn(stfd) {
...
int UdpListener::RecvFrom(void *buf, int size, ssize_t *nread,
                          struct sockaddr *from, int *fromlen) {
  return conn_->RecvFrom(buf, size, nread, from, fromlen);
}

int UdpListener::SendTo(void *buf, int size, ssize_t *nwrite,
...
UdpListener *ListenUdp(std::string local_ip, int local_port) {
  ...
  if ((_fd = socket(result->ai_family, result->ai_socktype,
    ...
    coco_error("create linux socket error. udp[%s:%d], ret=%d",
               local_ip.c_str(), local_port, ret);
    freeaddrinfo(result);
    return NULL;
  }
  coco_dbg("create linux socket success. udp[%s:%d], fd=%d", local_ip.c_str(),
           local_port, _fd);

  if (bind(_fd, result->ai_addr, result->ai_addrlen) == -1) {
    ret = ERROR_SOCKET_BIND;
    coco_error("bind socket error. udp[%s:%d], ret=%d", local_ip.c_str(),
               local_port, ret);
    freeaddrinfo(result);
    return NULL;
  }
  coco_dbg("bind socket success. udp[%s:%d], fd=%d", local_ip.c_str(),
           local_port, _fd);
