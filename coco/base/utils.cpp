...
int FastBuffer::realloc_buffer(int _size)
{
    ...
    if (realloc_size > MAX_SOCKET_BUFFER) {
        ret = ERROR_READER_BUFFER_OVERFLOW;
        coco_error("buffer overflow, required=%d, buffer size=%d, left=%d, realloc size=%d, max realloc size=%d",
    ...
    int nb_free_space = (int)(buffer + nb_buffer - end);
    coco_warn("buffer realloc, required=%d, before: max=%d, left=%d, now: max=%d, left=%d",
        _size, before_buffer_size, before_free_space, nb_buffer, nb_free_space);
    return ERROR_SUCCESS;
}

int write_large_iovs(IBufferWriter* skt, iovec* iovs, int size, ssize_t* pnwrite)
{
    int ret = ERROR_SUCCESS;
