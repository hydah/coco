#ifndef NET_HTTP_STACK_HPP
#define NET_HTTP_STACK_HPP
...
private:
    ...
    bool header_sent;
public:
    HttpResponseWriter(StSocket* io);
...
public:
    HttpResponseReader(HttpMessage* msg, StSocket* io);
...
public:
    ...
    virtual bool update_path(const std::string new_path) { path = new_path; return true; };
    virtual bool update_host(const std::string new_host) { host = new_host; return true; };
private:
    /**
...
public:
     ...
     */
    virtual int parse_message(StSocket* skt, HttpServerConn* conn, HttpMessage** ppmsg);
private:
    /**
     * parse the HTTP message to member field: msg.
     */
    virtual int parse_message_imp(StSocket* skt);
private:
...
public:
    HttpMessage(StSocket* io, HttpServerConn* c);
    virtual ~HttpMessage();
public:
    ...
    virtual int update(std::string url, http_parser* header,
        FastBuffer* body, std::vector<HttpHeaderField>& headers
    );
...
private:
    TcpConn* _conn;
    HttpServeMux* _mux;
    ConnMgr<HttpServerConn>* _mgr;
    HttpParser* _parser;

public:
    HttpServerConn(ConnMgr<HttpServerConn>* mgr, TcpConn* conn, HttpServeMux* mux);
    virtual ~HttpServerConn();

public:
    virtual int do_cycle();
    virtual void on_coroutine_stop();
    int process_request(HttpResponseWriter* w, HttpMessage* r);
};

class HttpServer : public ListenRoutine, public ConnMgr<HttpServerConn>
{
private:
    TcpListener* _l;
    HttpServeMux* _mux;
...
public:
    ...
    */
    virtual int initialize(std::string h, int p, int64_t t_us = HTTP_CLIENT_TIMEOUT_US, std::string url = "");
...
public:
    ...
    */
    virtual int post(std::string path, std::string req, HttpMessage** ppmsg, std::string request_id = "");
    /**
    ...
    * @param request_id request id, used for trace log.
    */
    virtual int get(std::string path, std::string req, HttpMessage** ppmsg, std::string request_id = "");
...
#endif // end of NET_HTTP_STACK_HPP