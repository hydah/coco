...
std::string generate_http_status_text(int status)
{
    ...
    if (_status_map.empty()) {
        ...
        _status_map[CONSTS_HTTP_NonAuthoritativeInformation    ] = CONSTS_HTTP_NonAuthoritativeInformation_str  ;
        ...
        _status_map[CONSTS_HTTP_ProxyAuthenticationRequired    ] = CONSTS_HTTP_ProxyAuthenticationRequired_str  ;
        ...
        _status_map[CONSTS_HTTP_RequestedRangeNotSatisfiable   ] = CONSTS_HTTP_RequestedRangeNotSatisfiable_str ;
...
int64_t HttpHeader::get_header_length()
{
    return header_length;
}

void HttpHeader::set_header_length(int64_t size)
{
    header_length = size;
}

std::string HttpHeader::content_type()
{
    return get("Content-Type");
}

void HttpHeader::set_content_type(std::string ct)
{
    set("Content-Type", ct);
}
...
HttpResponseWriter::HttpResponseWriter(StSocket* io)
{
    skt = io;
    hdr = new HttpHeader();
    header_wrote = false;
...
StSocket* HttpResponseWriter::st_socket()
{
    return skt;
}
...
int HttpResponseWriter::writev(iovec* iov, int iovcnt, ssize_t* pnwrite)
{
    ...
    // sendout all ioves.
    ssize_t nwrite;
    if ((ret = write_large_iovs(skt, iovss, nb_iovss, &nwrite)) != ERROR_SUCCESS) {
        return ret;
    }
...
int HttpResponseWriter::send_header(char* data, int size)
{
    ...
    // status_line
    ss << "HTTP/1.1 " << status << " "
        << generate_http_status_text(status) << HTTP_CRLF;
...
HttpResponseReader::HttpResponseReader(HttpMessage* msg, StSocket* io)
{
    skt = io;
    owner = msg;
    is_eof = false;
...
std::string HttpUri::get_uri_field(std::string uri, http_parser_url* hp_u, http_parser_url_fields field)
{
...
int HttpParser::parse_message(StSocket* skt, HttpServerConn* conn, HttpMessage** ppmsg)
{
    *ppmsg = NULL;
...
int HttpParser::parse_message_imp(StSocket* skt)
{
    int ret = ERROR_SUCCESS;
    ...
    //         // SRS_HTTP_CRLFCRLF "\r\n\r\n" // 0x0D0A0D0A
    //         if (p[0] == CONSTS_CR && p[1] == CONSTS_LF && p[2] == CONSTS_CR && p[3] == CONSTS_LF) {
    //             nparsed = http_parser_execute(&parser, &settings, buffer->bytes(), buffer->size());
    //             coco_info("buffer=%d, nparsed=%d, header=%d", buffer->size(), (int)nparsed, header_parsed);
    //             // break;
    //         }
    //     }
    ...
    //         coco_info("temp: size=%d,  %.*s", buffer->size(), buffer->size(), buffer->bytes());
    //     }

    //     // ok atleast header completed,
    //     // never wait for body completed, for maybe chunked.
    //     if (state == HttpParseStateHeaderComplete || state == HttpParseStateMessageComplete) {
    //         break;
    //     }
    ...
    while (true) {
        ...
        if (buffer->size() > 0) {
            ...
            coco_info("size=%d, nparsed=%d, consumed=%d", buffer->size(), (int)nparsed, consumed);
...
HttpMessage::HttpMessage(StSocket* io, HttpServerConn* c)
{
    conn = c;
    chunked = false;
...
int HttpMessage::update(std::string url_, http_parser* header, FastBuffer* body, std::vector<HttpHeaderField>& headers)
{
    int ret = ERROR_SUCCESS;
    ...
    if ((ret = _uri->initialize(uri_)) != ERROR_SUCCESS) {
        return ret;
    }
...
HttpResponseReader* HttpMessage::get_http_response_reader()
{
    return _body;
}

HttpServerConn* HttpMessage::connection()
{
    return conn;
}
...
int HttpMessage::body_read_all(std::string& body)
{
    ...
    while (!_body->eof()) {
        int nb_read = 0;
        if ((ret = _body->read(buf, HTTP_READ_CACHE_BYTES, &nb_read)) != ERROR_SUCCESS) {
            return ret;
        }
...
HttpResponseReader* HttpMessage::body_reader()
{
    return _body;
}
...
int HttpMessage::request_header_count()
{
...
bool IHttpHandler::is_not_found()
{
    return false;
}
...
bool HttpNotFoundHandler::is_not_found()
{
    return true;
}
...
int HttpServeMux::handle(std::string pattern, IHttpHandler* handler)
{
    ...
    if (entries.find(pattern) != entries.end()) {
        ...
        if (exists->explicit_match) {
            ...
            coco_error("http: multiple registrations for %s. ret=%d", pattern.c_str(), ret);
            return ret;
        }
    }
...
int HttpServeMux::serve_http(HttpResponseWriter* w, HttpMessage* r)
{
    ...
    if ((ret = find_handler(r, &handler)) != ERROR_SUCCESS) {
        coco_error("find handler failed. ret=%d", ret);
        return ret;
    }

    assert(handler);
    if ((ret = handler->serve_http(w, r)) != ERROR_SUCCESS) {
        if (!coco_is_client_gracefully_close(ret)) {
...
/* HttpServerConn */
HttpServerConn::HttpServerConn(ConnMgr<HttpServerConn>* mgr, TcpConn* conn, HttpServeMux* mux)
{
    _mgr = mgr;
    _conn = conn;
    _mux = mux;
...
int HttpServerConn::process_request(HttpResponseWriter* w, HttpMessage* r)
{
    int ret = ERROR_SUCCESS;

    coco_trace("HTTP %s %s, content-length=%ld", r->method_str().c_str(), r->url().c_str(), r->content_length());

    // use default server mux to serve http request.
    if ((ret = _mux->serve_http(w, r)) != ERROR_SUCCESS) {
        if (!coco_is_client_gracefully_close(ret)) {
...
int HttpServerConn::do_cycle()
{
    int ret = ERROR_SUCCESS;

    // initialize parser
    if ((ret = _parser->initialize(HTTP_REQUEST)) != ERROR_SUCCESS) {
        coco_error("api initialize http parser failed. ret=%d", ret);
        return ret;
    }

    // underlayer socket
    StSocket* skt = _conn->getStSocket();
    skt->set_recv_timeout(HTTP_RECV_TIMEOUT_US);

    // process http messages.
    while(!disposed) {
        HttpMessage* req = NULL;
        // get a http message
        if ((ret = _parser->parse_message(skt, this, &req)) != ERROR_SUCCESS) {
            return ret;
        }
        assert(req);
        ...
        while (!br->eof()) {
            if ((ret = br->read(buf, HTTP_READ_CACHE_BYTES, NULL)) != ERROR_SUCCESS) {
                return ret;
            }
        }
...
void HttpServerConn::on_coroutine_stop()
{
    _mgr->remove(this);
}
...
{
    _l = listen_tcp(local_ip, local_port);
    if (_l == NULL) {
...
int HttpServer::cycle()
{
     while(true) {
        ...
        HttpServerConn *pserver = new HttpServerConn(this, _conn, _mux);
        push(pserver);
...
int HttpClient::initialize(std::string _h, int p, int64_t t_us, std::string url)
{
    int ret = ERROR_SUCCESS;
...
int HttpClient::post(std::string path, std::string req, HttpMessage** ppmsg, std::string request_id)
{
    *ppmsg = NULL;
    ...
    if ((ret = connect()) != ERROR_SUCCESS) {
        coco_warn("http post. connect server failed. [host:%s, port:%d]ret=%d", host.c_str(), port, ret);
        return ret;
    }
    ...
    ss << "POST " << path << " "
        << "HTTP/1.1" << HTTP_CRLF
        << "Host: " << host << HTTP_CRLF
        << "Request-Id: " << request_id << HTTP_CRLF
        << "Connection: Keep-Alive" << HTTP_CRLF
        << "Content-Length: " << std::dec << req.length() << HTTP_CRLF
        << "User-Agent: " << "coco" << HTTP_CRLF
...
int HttpClient::get(std::string path, std::string req, HttpMessage** ppmsg, std::string request_id)
{
    *ppmsg = NULL;
...
int HttpClient::connect()
{
    ...
    // open socket.
    conn = dial_tcp(host, port, (int)timeout_us);
    if (conn == NULL) {
        coco_warn("http client failed, server=%s, port=%d, timeout=%ld", host.c_str(), port, timeout_us);
        return -1;
    }
    coco_info("connect to server success. server=%s, port=%d", host.c_str(), port);

    assert(!skt);
    skt =  conn->getStSocket();
    connected = true;
...
void HttpClient::set_https_flag(bool b_flag)
{
    is_https = b_flag;
}